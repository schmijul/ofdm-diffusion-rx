...
.PHONY: help venv install doctor quick-test test smoke train evaluate plot benchmark text-prior text-benchmark text-train-real text-benchmark-real fair-ablation-long fair-ablation-followup regime-compare regime-study-fast regime-study-fast-p8 regime-study-fast-p8-8seed regime-study-large regime-study-conference regime-study-smoke prior-sweep prior-sweep-large prior-sweep-smoke pilot-sweep frontend-compare summarize-regime ui clean
...
help:
	...
	@echo "  make fair-ablation-long - long resumable training + fair text ablation sweep"
	@echo "  make fair-ablation-followup - stronger focused follow-up run around the best fair setting"
	@echo "  make ui            - launch the local Streamlit control deck"
...
fair-ablation-long:
	$(VENV_PY) scripts/fair_ablation.py --config config/compare_text_real_long.yaml --outdir results/fair_long_run --train-texts data/grundgesetz.txt,data/text8.txt --max-bytes-per-text 2000000 --seeds 1,2,3,4,5 --diff-prior-weights 0.25,0.35,0.45 --max-bytes 20000 --grundgesetz-start-byte 0 --text8-start-byte 1000000

fair-ablation-followup:
	$(VENV_PY) scripts/fair_ablation.py --config config/compare_text_real_followup.yaml --outdir results/fair_followup --train-texts data/grundgesetz.txt,data/text8.txt --max-bytes-per-text 2000000 --seeds 1,2,3 --diff-prior-weights 0.45,0.55,0.65 --max-bytes 20000 --grundgesetz-start-byte 0 --text8-start-byte 1000000

ui:
	$(VENV_PY) -m streamlit run ui/app.py --server.address 127.0.0.1 --server.port $(UI_PORT)
